...
import ConjuredItem from "../app/UpdatableItems/ConjuredItem";
...
describe("Gilded Rose", function () {
  ...
  describe(`GildedRose with ${conjuredItemName} items`, () => {
    it("should reduce by 1 the sell in and by 2 the quality attributes", function () {
      const conjuredItem = new Item(conjuredItemName, 10, 10);
      const expectedItem = new ConjuredItem(conjuredItemName, 9, 8);
    ...
    it("should reduce by 4 the quality if the sellIn is zero or less", function () {
      const firstconjuredItem = new Item(conjuredItemName, 0, 10);
      const secondconjuredItem = new Item(conjuredItemName, -2, 10);
      const items = [firstconjuredItem, secondconjuredItem];
      const firstExpectedItem = new ConjuredItem(conjuredItemName, -1, 6);
      const secondExpectedItem = new ConjuredItem(conjuredItemName, -3, 6);
      const expectedItems = [firstExpectedItem, secondExpectedItem];
      const gildedRose = new GildedRose(items);
    ...
    it("should not reduce the quality of an item to a negative value", () => {
      const conjuredItem = new Item(conjuredItemName, 0, 0);
      const expectedItem = new ConjuredItem(conjuredItemName, -1, 0);
      const items = [conjuredItem];
      const gildedRose = new GildedRose(items);
